...
- **Traffic stress can be enriched later**: Casey Traffic Volume Survey adds observed vehicle volumes and 85th percentile speeds, but coverage is patchy across the footpath network. Treat as LGA-specific enriching data for future Accessibility refinement, not a v1.1 scoring input.
- **Road responsibility is strong dashboard context**: Casey Road Responsibility can show whether adjacent roads are local/collector or state-managed arterial/freeway/highway. This is useful for Council prioritisation and accountability, but should not directly score footpath quality.
...
| Public toilet facility locations  | Public Toilet Blocks (T1EAM)                                                        | Listed                          | Overlay in resident app + QA (3 Aug 2026); not in Day/Night index | Y                | Y                | Y                 | Overlay                       |
| Dog waste bag dispenser locations | Dog Dispenser Bags (T1EAM)                                                          | Listed                          | Overlay in resident app + QA (3 Aug 2026); not in index; portal export sparse (10) | Y                | Y                | Y                 | Overlay                       |
...
| **Footpaths (T1EAM)**                | City of Casey Open Data    | [View dataset](https://data.casey.vic.gov.au/explore/dataset/footpaths_ply_t1eam/)                                                                                                                                                       | LGA-specific essential | City of Casey                                   | Current; records include GIS modified date | Open Data (CC-BY-4.0)                                    | High – **walk network master** for ADR-008: segment geometry, `feature_type` (Footpath + Shared Use Path), surface, width, length, ownership, function/use, suburb/ward. 27,458 records (~3,239 shared use). Width outliers require QA. |
| **Shared Use Paths (T1EAM)**         | City of Casey Open Data    | [View dataset](https://data.casey.vic.gov.au/explore/dataset/sharedusepaths_ply_t1eam/)                                                                                                                                                  | LGA-specific essential | Casey + Cardinia (clip to Casey for pilot)      | Generated from footpaths; daily refresh metadata | Open Data (CC-BY-3)                                      | High – validation export for shared-use class; **not** row-unioned into master (subset of footpaths on `t1key`). Median width ~2.8 m vs ~1.5 m for standard footpaths. Crosswalk QA: `sharedusepaths_ply_t1eam_crosswalk.json`. |
...
| **Victorian Traffic Signals**        | Transport Victoria / DTP   | [DataVic page](https://discover.data.vic.gov.au/dataset/victorian-traffic-signals) / [CSV resource](https://discover.data.vic.gov.au/dataset/victorian-traffic-signals/resource/d094415e-7b73-414a-88f5-6a3a6b5a903d)                     | Base                   | Victoria-wide; spatially filter to Casey        | Published 2025; metadata modified 2026 | CC-BY-4.0 | Medium – identifies signalised intersections, pedestrian-operated signals, and flashing pedestrian crossings | Casey spatial review found ~203 signal assets in the LGA: ~156 intersections (`INT`), ~34 pedestrian-operated sites (`POS`), and ~13 flashing pedestrian crossings (`FLASH PX`). Signals are useful crossing context, but an intersection signal does not guarantee a legal or accessible pedestrian crossing on every approach. Proposed harmonisation: nearest / within 25–50 m to footpath segment, with `traffic_signal_type` and `traffic_signal_nearest_m`; use for QA/context until pedestrian crossing semantics are confirmed. |
| **Traffic Signal Configuration Data Sheets** | Transport Victoria / DTP | [DataVic page](https://discover.data.vic.gov.au/dataset/traffic-signal-configuration-data-sheets) | Base | Victoria-wide; join to Casey signal `SITE_NO` | Operation sheets published as redacted ZIP resources; metadata modified Mar 2026 | CC-BY-4.0 | Medium – may expose pedestrian signal groups, phasing, detectors, and time settings for signalised crossings | Dataset contains site-specific operation sheets split into large ZIPs by site-number range. Metadata defines `Time Settings` as phase and pedestrian time settings in seconds, plus signal groups, detectors, phasing, and operation notes. Casey signals span many ZIP ranges (~203 sites; site numbers 203–32079), including ~47 dedicated pedestrian-operated/flashing sites. Potential specialist QA source for whether signalised crossings provide adequate pedestrian clearance time, but too detailed and bulky for v1.1 ingest. Proposed future workflow: join `Victorian Traffic Signals.SITE_NO` to extracted operation sheets for Casey only, parse pedestrian signal groups/time settings where machine-readable, and validate manually before any scoring use. |
| **Traffic Volume Survey**            | City of Casey Open Data    | [View dataset](https://data.casey.vic.gov.au/explore/dataset/traffic-volume-survey-copy/)                                                                                                                                                | LGA-specific enriching | City of Casey survey sites                      | 2018–2026 survey years; portal updated June 2026 | Open Data (CC-BY-3) | Medium – observed vehicle volume and measured 85th percentile speed beside walking routes | 2,045 survey records / ~2,025 physical locations. Fields include `volume_24h`, `volume_12h`, `pc85th_kmh`, `comm_vhcl`, `peakvol`, `installed_year`, and point geometry. Investigation found good point-to-road alignment but patchy footpath coverage: ~11% of segments within 25 m, ~41% within 100 m. Proposed harmonisation: latest survey per physical location, join to footpath polygons by nearest point or 100 m corridor, expose `traffic_volume_24h`, `traffic_pc85_kmh`, `traffic_survey_year`, and distance/confidence fields. Candidate future Accessibility traffic-stress refinement; do not ingest or score in v1.1. |
| **Road Responsibility**              | City of Casey Open Data    | [View dataset](https://data.casey.vic.gov.au/explore/dataset/road-responsibility/)                                                                                                                                                       | LGA-specific enriching | City of Casey                                   | Portal metadata modified Sep 2024 | Open Data (CC-BY-3) | Medium – road class and ownership context next to footpath segments | 7,368 road records with `class_code`, `ownerhsip` (source typo), `road_classification`, `road_name`, suburb, ward, postcode, and geometry. Class counts are mainly Local Road (6,679), Collector Road (366), Sub-Arterial Road (111), Arterial Road (89), Freeway (68), and Highway (55). Spatial review found ~90% of footpath segments within 15 m of a road responsibility geometry. Proposed harmonisation: nearest/parallel road within 15–25 m, expose `adjacent_road_class_code`, `adjacent_road_owner`, `adjacent_road_name`, and distance. Best used as Council dashboard context and responsibility filter, not a direct v1.1 score input. |
...
| **Dog Dispenser Bags (T1EAM)**         | City of Casey Open Data | [View dataset](https://data.casey.vic.gov.au/explore/dataset/dog-dispenser-bags-pt-t1eam)                                                                                                                                                  | LGA-specific enriching | City of Casey                  | Current asset layer | Open Data (CC-BY-4.0) | Low–medium – supports dog walking route comfort                  | Portal export **10** records (3 Aug 2026; earlier register noted ~164 — confirm with Council if incomplete). Resident overlay + QA viewer; **not** in Day/Night index. |
...
| Shared Use Paths (T1EAM)  | [https://data.casey.vic.gov.au/explore/dataset/sharedusepaths_ply_t1eam/](https://data.casey.vic.gov.au/explore/dataset/sharedusepaths_ply_t1eam/)                                     |
...
| Traffic Volume Survey | [https://data.casey.vic.gov.au/explore/dataset/traffic-volume-survey-copy/](https://data.casey.vic.gov.au/explore/dataset/traffic-volume-survey-copy/) |
| Road Responsibility | [https://data.casey.vic.gov.au/explore/dataset/road-responsibility/](https://data.casey.vic.gov.au/explore/dataset/road-responsibility/) |
...
| Victorian Traffic Signals       | [https://discover.data.vic.gov.au/dataset/victorian-traffic-signals](https://discover.data.vic.gov.au/dataset/victorian-traffic-signals)         |
| Traffic Signal Configuration Data Sheets | [https://discover.data.vic.gov.au/dataset/traffic-signal-configuration-data-sheets](https://discover.data.vic.gov.au/dataset/traffic-signal-configuration-data-sheets) |
