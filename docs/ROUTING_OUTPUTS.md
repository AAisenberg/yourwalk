...
| Date | Change |
|------|--------|
| **17 Aug 2026** | Walk-view UX: footpath layer shows only genuine off-road paths (sidewalk lines — ADR-011 offsets, OSM-tagged and geometry-detected road-hugging footways — are hidden; they ringed every block and read as hollow polygons). Address pin now joins the route with a dotted grey connector instead of the walk line crossing yards. "Use this route" flies to a tilted street-level commence view facing the first leg, then toggles to overview. One-shot geolocate check-in button (no tracking; fix never leaves the device). |
